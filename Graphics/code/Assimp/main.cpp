...
void key_callback(GLFWwindow *window, int key, int scancode, int action, int mods) {
    ...
    if (key == GLFW_KEY_Z && action == GLFW_PRESS) {
        std::ofstream fd("info.txt");
        if (fd.fail()) {
            std::cout << "Failed to open file: " << "info.txt" << std::endl;
        }
        fd << objects.size() << "\n";
        for (int i = 0; i < objects.size(); ++i) {
            fd << object_names[i] << '\n';
            fd << objects[i].scale()[0] << '\n';
            fd << objects[i].translate()[0] << " ";
            fd << objects[i].translate()[1] << " ";
            fd << objects[i].translate()[2] << " " << '\n';
        }
        fd << cameras.size() << '\n';
        for (auto & camera : cameras) {
            ...
            fd << '\n';
            temp = camera.front_direction();
            for (int j = 0; j < 3; ++j) {
                fd << temp[j] << " ";
            }
            fd << '\n';
            temp = camera.up_direction();
            for (int j = 0; j < 3; ++j) {
                fd << temp[j] << " ";
            }
            fd << '\n';
    ...
    if (key == GLFW_KEY_X && action == GLFW_PRESS) {
        ...
        if (fin.fail())
            std::cout << "Failed to load file: " << "info.txt" << std::endl;
        int count;
        fin >> count;
...
void render_object() {
    ...
    if (g_is_perspective) {
        mat_proj = glm::perspective(glm::radians(cameras[cam_select_idx].fovy()), 1.0f, -1.0f, 1.0f);
    } else {
