...
void scroll_callback(GLFWwindow *window, double x, double y) {
    float fov = g_camera.fovy() - static_cast<float>(y);
    if (fov < 1.0f)
        fov = 1.0f;
    if (fov > 170.0f)
        fov = 170.0f;
...
void set_transform() {
    ...
    if (g_is_perspective) {
        ...
        mat_proj = mat_proj * glm::perspective(glm::radians(g_camera.fovy()), 1.0f, 0.1f, 200.0f);
    } else {
        g_camera.set_mode(Camera::kOrtho);
        mat_proj = mat_proj * glm::ortho(-1.0f, 1.0f, -1.0f, 1.0f, 0.1f, 200.0f);
    }

    mat_view = mat_view * g_camera.get_view_matrix();
